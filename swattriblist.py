...
def process_sattrib(gridfileslist,gridcols=None,gridheader=None,
...
def process_wscaletarget(csvfile,cmdlkind=None,
    ...
    else: #quantile distribution = normal
...
def process_wellattrib(cmdlwattribfile,cmdlwxyfile,
                    cmdlwattribcols=None,cmdlwxyzcols=None,
                    cmdlfillna=None,cmdloutdir=None):
...
def process_PCAanalysis(cmdlallattribcsv,cmdlacolsrange=None,
    ...
    if cmdlacolsrange:
        ...
        X = swa[swa.columns[cmdlacolsrange[0]: cmdlacolsrange[1]]].values
    else:
...
def process_PCAfilter(cmdlallattribcsv,cmdlacolsrange=None,cmdlanalysiscols=None,
                    cmdltargetcol=None,cmdlncomponents=None,cmdloutdir=None,cmdlcols2addback=None):
    swa = pd.read_csv(cmdlallattribcsv)
    if cmdlacolsrange:
        print('From col# %d to col %d' %(cmdlacolsrange[0],cmdlacolsrange[1]))
        X = swa[swa.columns[cmdlacolsrange[0]: cmdlacolsrange[1]]].values
        colnames = swa[swa.columns[cmdlacolsrange[0]: cmdlacolsrange[1]]].columns
    else:
        print('analysis cols',cmdlanalysiscols)
        X = swa[swa.columns[cmdlanalysiscols]].values
...
def process_eda(cmdlallattribcsv,
    ...
    def pltscattermatrix():
        ...
        sctrm = scatter_matrix(swax, alpha=0.2, figsize=(8, 8), diagonal='kde')
        if not cmdlhideplot:
            plt.show()
        plt.savefig(pdfcl)
        plt.close()
...
def process_linreg(cmdlallattribcsv,cmdlwtargetcol=None,
                cmdlwcolsrange=None,cmdlwpredictorcols=None,
                cmdlwellxyzcols=None,cmdlminmaxscale=None,
                cmdloutdir=None):
...
def process_TuneCatBoostRegressor(cmdlwellattribcsv,cmdlseisattribcsv,
                ...
                cmdllearningrate=None,cmdldepth=None,cmdlcv=None,
                cmdlhideplot=False):
...
def process_NuSVR(cmdlwellattribcsv,cmdlseisattribcsv,
                cmdlwcolsrange=None,cmdlwpredictorcols=None,
                cmdlwtargetcol=None,cmdlsaxyzcols=None,
                cmdlscolsrange=None,cmdlspredictorcols=None,
                cmdlminmaxscale=None,cmdloutdir=None,
                cmdlnu=None,
                cmdlerrpenalty=None,
                cmdlcv=None,cmdlscaleminmaxvalues =None,
                cmdlhideplot = False,
                cmdlvalsize=0.3,
                cmdlradius= None,
                cmdlinterpolate = 'idw', #hard coded interpolation method
...
#**********SGDR Regressor OLS/L1/L2 regresssion
def process_SGDR(cmdlwellattribcsv,cmdlseisattribcsv,
                cmdlwcolsrange=None,cmdlwpredictorcols=None,
                cmdlwtargetcol=None,cmdlsaxyzcols=None,
                cmdlscolsrange=None,cmdlspredictorcols=None,
                cmdlminmaxscale=None,cmdloutdir=None,
                cmdlloss=None, #‘squared_loss’, ‘huber’, ‘epsilon_insensitive’, or ‘squared_epsilon_insensitive’
                cmdlpenalty= 'l2',#options: l1,l2,elasticnet,none
                cmdll1ratio = 0.15, #elastic net mixing: 0 (l2)to 1 (l1)
                cmdlcv=None,cmdlscaleminmaxvalues =None,
                cmdlhideplot = False,
                cmdlvalsize=0.3,
                cmdlradius= None,
...
def process_CatBoostRegressor(cmdlwellattribcsv,cmdlseisattribcsv,
                ...
                cmdlscolsrange=None,cmdlspredictorcols=None,
                cmdlminmaxscale=None,cmdloutdir=None,
                cmdliterations=None,cmdllearningrate=None,
                cmdldepth=None,cmdlcv=None,cmdlscaleminmaxvalues =None,
    ...
    #does not work: problem with eval_set variable Mar 1 2018
        ...
        if not cmdlgeneratesamples:
            ...
            #commenting out annotation : only shows on last plot!!
            # plt.annotate('%s = %-.*f   + %-.*f * %s' % (ytitle,dp,qc[0],dp,qc[1],xtitle),\
                        # xy =(yvi[4],xvi[4]),xytext=(0.25,0.80),textcoords='figure fraction')
...
def process_CatBoostClassifier(cmdlwellattribcsv,cmdlseisattribcsv,
                ...
                cmdlminmaxscale=None,cmdloutdir=None,cmdliterations=None,
                cmdllearningrate=None,cmdldepth=None,cmdlqcut=None,
                cmdlcv=None,cmdlfeatureimportance=False,
                cmdlgeneratesamples = None,
        ...
        if not cmdlgeneratesamples:
            ...
            with PdfPages(pdfcl) as pdf:
                for i in range(0,swxyz1.shape[0],20):
                    ...
                    ax =swtemp[['qcodes','predqcodes']].plot(kind='bar',rot=45)
                    pdf.savefig()
                    if not cmdlhideplot:
                        plt.show()
                    plt.close()
...
def process_TuneCatBoostClassifier(cmdlwellattribcsv,cmdlseisattribcsv,
    ...
    if cmdlscolsrange:
        ...
        Xpred = ssa[ssa.columns[cmdlscolsrange[0]: cmdlscolsrange[1]]].values
        colnames = ssa[ssa.columns[cmdlscolsrange[0]: cmdlscolsrange[1]]].columns
    else:
        print('Seismic Predictor cols',cmdlspredictorcols)
        Xpred = ssa[ssa.columns[cmdlspredictorcols]].values
...
def process_GaussianNaiveBayes(cmdlwellattribcsv,cmdlseisattribcsv,
                cmdlwcolsrange=None,cmdlwanalysiscols=None,
                cmdlqcut=None,cmdlwellsxyzcols=None,
                cmdlsaxyzcols=None,cmdlscolsrange=None,
                cmdlspredictorcols=None,
                cmdlwtargetcol=None,
                cmdlcoded=None,
...
def process_NuSVC(cmdlwellattribcsv,cmdlseisattribcsv,
    ...
    if cmdlcv:
        ...
        #[(ssa[probacolnames[i]] = allprob[:,i]) for i in range(cmd.qcut)]
        # for i in range(cmdlqcut):
        for i in range(nclasses):   #************need to adjust this for all other classifications
            ssa[probacolnames[i]] = allprob[:,i]
            ssxyz[probacolnames[i]] = allprob[:,i]
...
def process_GaussianMixtureModel(cmdlwellattribcsv,cmdlseisattribcsv,
    ...
    if cmdlwcolsrange:
        ...
        colnames = swa[swa.columns[cmdlwanalysiscols]].columns
    if cmdlwtargetcol:
        inclasses= swa[swz.columns[cmdlwtargetcol]].unique().tolist()
...
def process_clustering(cmdlallattribcsv,cmdlcolsrange=None,
                cmdlcols2cluster=None,cmdlnclusters=None,
                cmdlplotsilhouette=False,cmdlsample=None,cmdlxyzcols=None,
                cmdladdclass=None,cmdloutdir=None,cmdlhideplot=False):
...
def process_dbscan(cmdlallattribcsv,cmdlcolsrange=None,
                cmdlcols2cluster=None,cmdlsample=None,
                cmdlxyzcols=None,cmdlminsamples=None,
                cmdladdclass=None,cmdleps=None,cmdloutdir=None):
...
def process_tSNE2(cmdlwellattribcsv,cmdlseisattribcsv,
                cmdlwcolsrange=None,cmdlwpredictorcols=None,
                cmdlwtargetcol=None,cmdlsxyzcols=None,
                cmdlscolsrange=None,cmdlspredictorcols=None,
                cmdlsample=None,
                cmdlwxyzcols=None,
                cmdllearningrate=None,
...
def getcommandline(*oneline):
    ...
    lrparser = subparser.add_parser('featureranking',help='Ranking of attributes')
    lrparser.set_defaults(which = 'featureranking')
    lrparser.add_argument('allattribcsv',help='csv file with all predictors and target column')
    lrparser.add_argument('--wellxyzcols',type=int,nargs=4,default=[0,1,2,3],
                            help='Columns well x y z  to remove from fitting model. default= 0 1 2 3 ')
    lrparser.add_argument('--wtargetcol',type=int, default =-1,help='Target column #.default = last col')
    lrparser.add_argument('--wpredictorcols',type=int,nargs='+',help='Predictor column #s, no default')
    lrparser.add_argument('--wcolsrange',type=int,nargs=2,default=None,help='Wells Predictor min max col #')
    lrparser.add_argument('--testfeatures',choices=['rfe','rlasso','svr','svrcv','rfregressor','decisiontree'],default='rfregressor',
    ...
    lrparser.add_argument('--lassoalpha',type=float,default = 0.025,help='alpha = 0 is OLS. default=0.005')
    #features2keep is used with svr only
    lrparser.add_argument('--features2keep',type=int,default=5,help='#of features to keep.default=5')
    #following 2 are used with any cross validation e.g. random forest regressor, svrcv
    lrparser.add_argument('--cv',type = int,default = 3,help='Cross Validation nfold. default=3')
    lrparser.add_argument('--traintestsplit',type=float,default =.3,help='Train Test split. default = 0.3')
...
def main():
    ...
    def process_commands():
        ...
        if cmdl.which == 'sattrib':
            process_sattrib(cmdl.gridfileslist,gridcols = cmdl.gridcols,
                            gridheader = cmdl.gridheader,
                            ilxl=cmdl.ilxl,outdir = cmdl.outdir)

        elif cmdl.which =='dropcols':
            process_dropcols(cmdl.csvfile,cmdlcols2drop=cmdl.cols2drop,cmdloutdir=cmdl.outdir)
            ...
            process_wscaletarget(cmdl.csvfile,cmdlkind=cmdl.kind,
            ...
            process_wellattrib(cmdl.wattribfile,cmdl.wxyfile,
                            cmdlwattribcols=cmdl.wattribcols,cmdlwxyzcols=cmdl.wxyzcols,
                            cmdlfillna=cmdl.fillna,cmdloutdir=cmdl.outdir)
            ...
            process_seiswellattrib(cmdl.seiscsv,cmdl.wellcsv,
                            cmdlwellcsvcols=cmdl.wellcsvcols,cmdlradius=cmdl.radius,
                            cmdlinterpolate=cmdl.interpolate,cmdloutdir=cmdl.outdir)


        elif cmdl.which =='PCAanalysis':
            process_PCAanalysis(cmdl.allattribcsv,cmdlacolsrange=cmdl.acolsrange,
            ...
            process_PCAfilter(cmdl.allattribcsv,cmdlacolsrange=cmdl.acolsrange,
                            cmdlanalysiscols=cmdl.analysiscols,
                            cmdltargetcol=cmdl.targetcol,
            ...
            process_scattermatrix(cmdl.allattribcsv,cmdlwellxyzcols=cmdl.wellxyzcols,
            ...
            process_linreg(cmdl.allattribcsv,cmdlwtargetcol=cmdl.wtargetcol,
                            cmdlwcolsrange=cmdl.wcolsrange,cmdlwpredictorcols=cmdl.wpredictorcols,
                            cmdlwellxyzcols=cmdl.wellxyzcols,cmdlminmaxscale=cmdl.minmaxscale,
                            cmdloutdir=cmdl.outdir)
            ...
            #print(swa.head(5))
            print('Well Target: %d ' % cmdl.wtargetcol)
            if cmdl.wcolsrange:
                print('Well Predictors From col# %d to col %d' %(cmdl.wcolsrange[0],cmdl.wcolsrange[1]))
                X = swa[swa.columns[cmdl.wcolsrange[0]: cmdl.wcolsrange[1]]].values
                colnames = swa[swa.columns[cmdl.wcolsrange[0]: cmdl.wcolsrange[1]]].columns
            else:
                print('Well Predictor cols',cmdl.wpredictorcols)
                X = swa[swa.columns[cmdl.wpredictorcols]].values
                colnames = swa[swa.columns[cmdl.wpredictorcols]].columns
            y = swa[swa.columns[cmdl.wtargetcol]]

            swxyz = swa[swa.columns[cmdl.wellxyzcols]].copy()


            if cmdl.testfeatures == 'rlasso':
                rlasso = RandomizedLasso(alpha=cmdl.lassoalpha)
                rlasso.fit(X, y)
            ...
            elif cmdl.testfeatures =='rfe':
                #rank all features, i.e continue the elimination until the last one
                lm = LinearRegression()
                rfe = RFE(lm, n_features_to_select=cmdl.features2keep)
                rfe.fit(X,y)
                ...
                for i in range(X.shape[1]):
                     score = cross_val_score(lm, X[:, i:i+1], y, scoring="r2",
                                              cv=ShuffleSplit(len(X), cmdl.cv, cmdl.traintestsplit))
            ...
            elif cmdl.testfeatures =='svrcv':
                #rank all features, i.e continue the elimination until the last one

                estimator = SVR(kernel="linear")
                selector = RFECV(estimator, step=1,  cv = cmdl.cv)
            ...
            elif cmdl.testfeatures =='svr':
                estimator = SVR(kernel="linear")
                selector = RFE(estimator, cmdl.features2keep, step=1)
                selector = selector.fit(X, y)
                fr = pd.DataFrame(sorted(zip(selector.ranking_, colnames)),columns=['Importance','Attribute'])
            ...
            elif cmdl.testfeatures =='rfregressor':
            ...
            elif cmdl.testfeatures =='decisiontree':
            ...
            process_linfitpredict(cmdl.wellattribcsv,cmdl.seisattribcsv,
                            cmdlwcolsrange=cmdl.wcolsrange,cmdlwpredictorcols=cmdl.wpredictorcols,
                            cmdlwtargetcol=cmdl.wtargetcol,cmdlsaxyzcols=cmdl.saxyzcols,
                            cmdlscolsrange=cmdl.scolsrange,cmdlspredictorcols=cmdl.spredictorcols,
                            cmdlminmaxscale=cmdl.minmaxscale,cmdloutdir=cmdl.outdir,
                            cmdlhideplot=cmdl.hideplot)
            ...
            swa = pd.read_csv(cmdl.allattribcsv)
            swa = swa.sample(frac=cmdl.sample).copy()

            if cmdl.wcolsrange:
                print('Predictors From col# %d to col %d' %(cmdl.wcolsrange[0],cmdl.wcolsrange[1]))
                X = swa[swa.columns[cmdl.wcolsrange[0]: cmdl.wcolsrange[1]]].values
                colnames = swa[swa.columns[cmdl.wcolsrange[0]: cmdl.wcolsrange[1]]].columns
            else:
                print('Predictor cols',cmdl.predictorcols)
                X = swa[swa.columns[cmdl.predictorcols]].values
                colnames = swa[swa.columns[cmdl.predictorcols]].columns
            y = swa[swa.columns[cmdl.wtargetcol]]
            ...
            dirsplit,fextsplit= os.path.split(cmdl.allattribcsv)
            fname,fextn= os.path.splitext(fextsplit)
            if cmdl.outdir:
                pdfcl = os.path.join(cmdl.outdir,fname) +"_knn.pdf"
            else:
                pdfcl = os.path.join(dirsplit,fname) +"_knn.pdf"

            #with PdfPages(pdfkcl) as pdf:
            #plt.figure(figsize=(8,8))
            ...
            process_TuneCatBoostRegressor(cmdl.wellattribcsv,cmdl.seisattribcsv,
                                ...
                                cmdlwtargetcol=cmdl.wtargetcol,cmdlsaxyzcols=cmdl.saxyzcols,
                                cmdlscolsrange=cmdl.scolsrange,cmdlspredictorcols=cmdl.spredictorcols,
                                cmdlwellsxyzcols=cmdl.wellsxyzcols,cmdlminmaxscale=cmdl.minmaxscale,
                                cmdloutdir=cmdl.outdir,cmdliterations = cmdl.iterations,
            ...
            process_CatBoostRegressor(cmdl.wellattribcsv,cmdl.seisattribcsv,
                                ...
                                cmdlminmaxscale=cmdl.minmaxscale,
                                cmdloutdir=cmdl.outdir,cmdliterations = cmdl.iterations,
                                cmdllearningrate=cmdl.learningrate,cmdlscaleminmaxvalues =cmdl.scaleminmaxvalues,
                                cmdldepth = cmdl.depth,cmdlcv =cmdl.cv,
            ...
            process_CatBoostClassifier(cmdl.wellattribcsv,cmdl.seisattribcsv,
                                ...
                                cmdloutdir=cmdl.outdir,cmdliterations = cmdl.iterations,
                                cmdllearningrate=cmdl.learningrate,cmdlcoded = cmdl.coded,
                                cmdldepth = cmdl.depth,cmdlqcut=cmdl.qcut,
                                cmdlcv =cmdl.cv,cmdlvalsize=cmdl.valsize,
                                cmdlfeatureimportance = cmdl.featureimportance,
                                cmdlgeneratesamples = cmdl.generatesamples,
            ...
            process_TuneCatBoostClassifier(cmdl.wellattribcsv,cmdl.seisattribcsv,
                                ...
                                cmdlwtargetcol=cmdl.wtargetcol,cmdlsaxyzcols=cmdl.saxyzcols,
                                cmdlscolsrange=cmdl.scolsrange,cmdlspredictorcols=cmdl.spredictorcols,
                                cmdlwellsxyzcols=cmdl.wellsxyzcols,cmdlcoded = cmdl.coded,
                                cmdloutdir=cmdl.outdir,cmdliterations = cmdl.iterations,
                                cmdllearningrate=cmdl.learningrate,
                                cmdldepth = cmdl.depth,cmdlqcut=cmdl.qcut,cmdlcv=cmdl.cv,
                                cmdlhideplot=cmdl.hideplot)
            ...
            swa = pd.read_csv(cmdl.wellattribcsv)
            if cmdl.wcolsrange:
                print('Well From col# %d to col %d' %(cmdl.wcolsrange[0],cmdl.wcolsrange[1]))
                X = swa[swa.columns[cmdl.wcolsrange[0]: cmdl.wcolsrange[1]]].values
                colnames = swa[swa.columns[cmdl.wcolsrange[0]: cmdl.wcolsrange[1]]].columns
            else:
                print('Well analysis cols',cmdl.wanalysiscols)
                X = swa[swa.columns[cmdl.wanalysiscols]].values
                colnames = swa[swa.columns[cmdl.wanalysiscols]].columns

            probacolnames = ['Class%d'%i for i in range(cmdl.qcut)]
            swa['qa'],qbins = pd.qcut(swa[swa.columns[cmdl.wtargetcol]],cmdl.qcut,labels=probacolnames,retbins=True)
            swa['qcodes'] = swa['qa'].cat.codes
            y = swa['qcodes'].values
            ...
            process_GaussianNaiveBayes(cmdl.wellattribcsv,cmdl.seisattribcsv,
                            ...
                            cmdlscolsrange=cmdl.scolsrange,
                            cmdlspredictorcols=cmdl.spredictorcols,
                            cmdlwtargetcol=cmdl.wtargetcol,cmdlcoded = cmdl.coded,
                            cmdloutdir=cmdl.outdir,cmdlcv = cmdl.cv,cmdlvalsize=cmdl.valsize,
                            cmdlgeneratesamples = cmdl.generatesamples,
                            cmdlbalancetype = cmdl.balancetype,
                            cmdlnneighbors = cmdl.nneighbors,
                            cmdlhideplot=cmdl.hideplot)
            ...
            process_NuSVC(cmdl.wellattribcsv,cmdl.seisattribcsv,
                            cmdlwcolsrange=cmdl.wcolsrange,cmdlwanalysiscols=cmdl.wanalysiscols,
                            cmdlqcut=cmdl.qcut,cmdlwellsxyzcols=cmdl.wellsxyzcols,
                            cmdlsaxyzcols=cmdl.saxyzcols,cmdlscolsrange=cmdl.scolsrange,
                            cmdlspredictorcols=cmdl.spredictorcols,
                            cmdlwtargetcol=cmdl.wtargetcol,
                            cmdlcoded = cmdl.coded,
                            cmdloutdir=cmdl.outdir,cmdlcv = cmdl.cv,
                            cmdlvalsize=cmdl.valsize,
                            cmdlnu = cmdl.nu,
                            cmdlbalancetype = cmdl.balancetype,
                            cmdlnneighbors = cmdl.nneighbors,
            ...
            if cmdl.colsrange:
                print('Well From col# %d to col %d' %(cmdl.colsrange[0],cmdl.colsrange[1]))
                swax = swa[swa.columns[cmdl.colsrange[0]: cmdl.colsrange[1]]]
            else:
                swax = swa[swa.columns[cmdl.cols2cluster]]
            swax = swax.sample(frac=cmdl.sample).copy()
            ...
            process_clustering(cmdl.allattribcsv,cmdlcolsrange=cmdl.colsrange,
                            cmdlcols2cluster=cmdl.cols2cluster,cmdlnclusters=cmdl.nclusters,
                            cmdlplotsilhouette=cmdl.plotsilhouette,cmdlsample=cmdl.sample,cmdlxyzcols=cmdl.xyzcols,
                            cmdladdclass=cmdl.addclass,cmdloutdir=cmdl.outdir,
                            cmdlhideplot=cmdl.hideplot)
            ...
            process_dbscan(cmdl.allattribcsv,cmdlcolsrange=cmdl.colsrange,
                        cmdlcols2cluster=cmdl.cols2cluster,
                        cmdlxyzcols=cmdl.xyzcols,cmdlminsamples=cmdl.minsamples,
                        cmdladdclass=cmdl.addclass,cmdleps=cmdl.eps,
                        cmdloutdir=cmdl.outdir)
